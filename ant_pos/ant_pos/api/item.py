...
@frappe.whitelist()
def get_item_details_from_input(pos_profile, search_value, customer):
    if not customer:
...
def items(pos_profile=None, search_value="", customer=None):
    ...
    for item in items_list:
        item["stock_qty"] = frappe.db.get_value(
            "Bin",
            {"item_code": item["item_code"], "warehouse": get_pos_warehouse(pos_profile)},
            "actual_qty"
        ) or 0
